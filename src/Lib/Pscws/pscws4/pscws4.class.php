...
{	
	...
	function get_value()
	{
		...
		while (($off = ($off + $clen)) < $len)
		{
			...
			elseif (($pflag & PSCWS4_PFLAG_WITH_MB) && $clen === 1) {
		...
		elseif ($pflag & PSCWS4_PFLAG_WITH_MB) $this->_msegment($off, $zlen);
		elseif (!($pflag & PSCWS4_PFLAG_ALNUM) || (($off - $this->_off) >= PSCWS4_MAX_EWLEN)) $this->_ssegment($off);
		else
		{
			$zlen = $off - $this->_off;
	...
	function _rule_load($fpath)
	{
		...
		while ($buf = fgets($fd, 512))
		{
			...
			elseif ($key === 'nostats') $item['bit'] = PSCWS4_RULE_NOSTATS;
			else 
			{
				$item['bit'] = (1<<$j);
				$j++;
		...
		while ($buf = fgets($fd, 512))
		{
			...
			if ($ch === ':') {
				...
				elseif ($pkey === 'tf') $item['tf'] = floatval($pval);
				elseif ($pkey === 'idf') $item['idf'] = floatval($pval);
				elseif ($pkey === 'attr') $item['attr'] = $pval;	// 2bytes?
				elseif ($pkey === 'znum') {
					if ($pos = strpos($pval, ',')) {
				...
				elseif ($pkey === 'include' || $pkey === 'exclude') {
					$clude = 0;
	...
	function _ssegment($end)
	{
		...
		while ($start < $end)
		{
			...
			elseif (!($this->_mode & PSCWS4_IGN_SYMBOL)) {
