 ...
 * 表单项 文件
 */
class FormItemStorageFile extends FormItem
{
    ...
    public function __construct($params = [], $row = [])
    {
        ...
        if (!isset($this->ui['@click'])) {
            $this->ui['@click'] = 'formItemStorageFile_' . $this->name . '_select';
        }
    ...
    public function getHtml()
    {
        ...
        $html .= '<el-link type="danger" icon="el-icon-delete" @click="formItemStorageFile_' . $this->name . '_delete"></el-link>';
        $html .= '</div>';
    ...
    {
        return 'function formItemStorageFile_' . $this->name . '_selected(files){vueForm.formItemStorageFile_' . $this->name . '_selected(files);}';
    }
    ...
    {
        $imageCallback = base64_encode('parent.be.getActiveIframe().formItemStorageFile_' . $this->name . '_selected(files);');
        $iframeUrl = beAdminUrl('System.Storage.pop', ['callback' => $imageCallback]);

        $imageCallback = base64_encode('parent.formItemStorageFile_' . $this->name . '_selected(files);');
        $url = beAdminUrl('System.Storage.pop', ['callback' => $imageCallback]);

        return [
            'formItemStorageFile_' . $this->name . '_select' => 'function () {
            ...
            'formItemStorageFile_' . $this->name . '_selected' => 'function (files) {
                if (files.length > 0) {
                    let file = files[0];
            ...
            'formItemStorageFile_' . $this->name . '_delete' => 'function () {
