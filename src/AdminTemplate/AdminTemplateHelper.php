...
class AdminTemplateHelper
{
    ...
    {
        $themeProperty = Be::getProperty( 'AdminTheme.' . $theme);
        $runtime = Be::getRuntime();
        $fileTheme = $runtime->getRootPath() . $themeProperty->getPath() . '/' . $theme . '.php';
        if (!file_exists($fileTheme)) {
            throw new RuntimeException( 'AdminTheme ' . $theme . ' doesn\'t exist!');
        }
        ...
        if (preg_match_all('/<be-include>(.*?)<\/be-include>/s', $contentTemplate, $matches)) {
            ...
            foreach ($matches[1] as $m) {
                ...
                if (count($includes) > 2) {
                    ...
                    $fileInclude = $runtime->getRootPath() . $tmpProperty->getPath() . '/AdminTemplate/' . implode('/', $includes) . '.php';
                    if (!file_exists($fileInclude)) {
                        // 模板中包含的文件 $m 不存在
                        throw new RuntimeException('AdminTemplate include file ' . $m . ' doesn\'t exist!');
                    }
